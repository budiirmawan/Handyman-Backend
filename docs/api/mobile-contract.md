...
- Every response carries `X-Request-ID`. The value is **server-generated and
  server-authoritative**: the backend always mints a fresh UUID at request
  entry and never adopts or echoes a client-supplied `X-Request-ID` header
  (correlation ids must not be caller-chosen; see CR-BE-AUDIT-01). The same
  value appears on the response header and in `error.requestId`. Mobile logs
  it for support correlation. This is request **correlation**, not an
...
| Form instance mobile execution (MOB-C07) | `POST /mobile/tasks/{taskId}/form-instance`, `POST /mobile/form-instances/{instanceId}/start`, `PUT …/responses`, `POST …/complete`, `POST …/cancel` |
...
- **Shift context (MOB-C04 PART 01)** — every item carries a nullable `shift`
  object (`assignmentId`, `shiftId`, `code`, `name`, `startTime`, `endTime`,
  `securityPost`). It is populated only when the item's Building is the Building
  the worker is currently on shift at, resolved by reusing the authoritative
  `/mobile/current-shift` derivation (BE-03E roster + BE-02F/G Building scope +
  Building-timezone wall-clock). It is an annotation for execution scoping, not
  a filter and never a client-computed "active shift" flag: a `null` shift
  leaves the item in the feed. BE-02G scope isolation is the only authority
  that removes work from the response.
- **Shift projection (MOB-C04 PART 01A)** — two modes on the same endpoint:
  - **Default** — `GET /mobile/assignments` returns **all** accessible active
    assignments (on-shift or not, incl. planned/assigned work). BE-25C is not
    redefined.
  - **`GET /mobile/assignments?shift=current`** — the now-live current-shift
    projection: only items whose Building is a Building the worker is currently
    on shift at (authoritative `/mobile/current-shift` authority) = the
    currently-executable field work for the worker's current shift. When the
    worker has no current shift this returns `200` with an empty `data` array —
    it never falls back to the all-accessible surface. `securityPost` remains
    context-only (never a filter). `current` is the only supported value; any
    other value is `400 VALIDATION_ERROR`. `buildingId`/`shiftId`/
    `securityPostId` supplied by the client are never authority.
...
### Mobile online execution lock (MOB-C04 PART 02)

Mobile executes a checklist **online** through the mobile checklist command
surface (each composes the SAME shared checklist-execution service as the
generic REST execution endpoints — no separate mobile engine):

- `POST /mobile/checklist-executions/{executionId}/start`
- `PUT  /mobile/checklist-executions/{executionId}/responses`
- `POST /mobile/checklist-executions/{executionId}/complete`
- `POST /mobile/checklist-executions/{executionId}/cancel`

Each mutation is additionally gated by the **authoritative work + current-shift
Building authority** (server-derived; never client-supplied):

- **Authoritative task binding (MOB-C04 PART 02A).** A checklist execution is
  field work only when it carries an authoritative `generated_task_id`
  (nullable `checklist_executions.generated_task_id` → `generated_tasks`).
  The execution's work Building is resolved from that **exact** generated task
  (`generated_task_id → generated_tasks.building_id`) — never from "the first
  generated task matching the checklist template" (that projection is
  ambiguous: one template can be targeted by many tasks across many
  Buildings/workers/occurrences and is not an authorization grant).
- **Unbound / standalone execution.** A checklist execution with no
  `generated_task_id` (historical or admin-created standalone) is NOT field
  work: it receives no field-work authorization grant and its mobile mutation
  **fails closed** with `403 CHECKLIST_EXECUTION_NOT_IN_CURRENT_SHIFT`. It never
  guesses a task by template. Generic Web/admin execution of standalone
  executions is unchanged.
- **On shift + assigned + bound** — the command proceeds through the shared
  service (lifecycle/state rules apply normally).
- **Accessible but off-shift / no current shift / not the assigned worker /
  unbound** — rejected with `403 CHECKLIST_EXECUTION_NOT_IN_CURRENT_SHIFT`.
- **Inaccessible execution** — rejected with the existing `403
  BUILDING_ACCESS_DENIED` (no existence leak).
- Worker authority reuses the existing task-assignment authority: the bound
  generated task must be assigned to the caller's Workforce Profile (direct
  `WORKFORCE`) or the caller's Team (`TEAM`).
- The server determines the Building from `generated_task_id` and the
  shift/assignment authority from the session. **No client-provided**
  `buildingId` / `shiftId` / `securityPostId` / `role` / `permission` /
  `activeShift` / `workforceProfileId` / `generatedTaskId` is authority.
- Read behavior (`GET /mobile/checklist-executions/{executionId}`) remains
  scoped as before: for a **bound** execution the `task` reference is the exact
  bound task (authoritative); for an unbound execution the `task` field stays a
  best-effort display and is never an authorization grant. Mutations of
  off-shift work stay locked.

### Task → checklist-execution open (MOB-C04 PART 02B)

`POST /mobile/tasks/{taskId}/checklist-execution` opens (get-or-create) the
authoritative bound checklist execution for a generated task the authenticated
worker is authorized to execute. The `taskId` is the mobile-assignment feed
`reference.taskId` for a `TASK` item. It is the **authority entry point** for a
field checklist: it does not accept an arbitrary template.

Authority chain (all server-derived, never client-supplied):
- the generated task exists,
- its `target_type` is `CHECKLIST_TEMPLATE`,
- `target_id` references an **ACTIVE** checklist template,
- the task's Building is inside the caller's BE-02G accessible scope,
- the task is executable by the caller via the existing **WORKFORCE / TEAM**
  assignment authority,
- the caller is currently on shift in the task's Building.

**Get-or-create (idempotent):** if a checklist execution is already bound to the
task (`generated_task_id`) it is returned unchanged; otherwise exactly one is
created bound with `generated_task_id = taskId`, `checklist_template_id = task
target_id`, `client_id = task client`. A partial UNIQUE index prevents
duplicates under concurrent requests. Response is the minimum reference to
continue: `{ executionId, generatedTaskId, checklistTemplateId, clientId,
status }`. It only establishes the binding — the worker then drives
`start`/`responses`/`complete`/`cancel` through the shared service and the
PART 02 shift-locked commands, and reads full content via the BE-25D read
model. Client-provided `templateId`/`buildingId`/`shiftId`/`securityPostId`/
`workforceProfileId` are never authority.

## 7c2. Form Instance mobile execution (MOB-C07)

Authoritative field Form lifecycle (FORM_VERSION only):

```text
FORM_VERSION generated task
→ one bound Form Instance (`generated_task_id`)
→ start / responses
→ evidence (`evidence.manage`, non-terminal instance)
→ complete or cancel (does not mutate the generated task)
→ review (`review.manage` / `review.read`, COMPLETED instance)
```

- `POST /mobile/tasks/{taskId}/form-instance` — get-or-create the bound DRAFT
  instance. `taskId` is the assignment-feed `reference.taskId`. Target must
  be **FORM_VERSION** with that exact PUBLISHED version and an ACTIVE parent
  template. `FORM_TEMPLATE` fails closed (no latest-PUBLISHED resolve).
...
Every Form mutation requires `form_instance.execute` **and** re-checks:
bound `generated_task_id` (unbound generic instances stay on generic
routes), exact FORM_VERSION task, pinned version PUBLISHED, parent ACTIVE,
Client consistency, BE-02G Building, ACTIVE workforce + WORKFORCE/TEAM
assignment, current shift, task `OPEN|ASSIGNED|IN_PROGRESS`. Body ids are
never authority. Lifecycle is the shared PART 01A service (DRAFT complete
when required fields are present). Completing/cancelling the Form does
**not** mutate generated-task status. Evidence and review keep their own
permissions and also do not mutate the task.
...
Upload permission is exact per `executionType` (MOB-C06 PART 01/01A,
asserted server-side after the multipart body is parsed — never accepted
from the client): `FORM_INSTANCE` / `CHECKLIST_EXECUTION` / `FINDING` /
`FINDING_REWORK` → `evidence.manage`; `FINDING_VERIFICATION` →
`finding.review` (the dedicated verification-evidence authority —
`evidence.manage` alone does NOT grant it, and a reviewer is NOT required
to also hold `evidence.manage`). No current-shift requirement is involved.
`GET /mobile/evidence/{evidenceId}` requires `evidence.read`.
...
- `building` — Building context when resolvable through the target task
  (`{id, code, name}`, else `null`). For the Finding-related kinds
  (MOB-C06 PART 02) it comes from the parent Finding's building and is
  only returned when that Building is accessible to the caller,
...
- `target` — `{executionType, executionId, checklist|null, form|null,
  task|null, finding|null, rework|null, verification|null}` (checklist
  `task` = first generated task targeting the checklist template). For a
  task-bound `FORM_INSTANCE` (MOB-C07 PART 05) `task`/`building` come from
  the exact `generated_task_id` (FORM_VERSION pin + Client consistency);
  unbound generic instances stay Client-scoped with `task`/`building` null.
  MOB-C06 PART 02: for
  FINDING / FINDING_REWORK / FINDING_VERIFICATION evidence the server
  derives, from the STORED evidence only (never from client input), a
  minimal Finding reference `finding` (`{id, findingNumber, title,
  status}`), plus `rework` (`{id, status}`) for FINDING_REWORK and
  `verification` (`{id, status}`) for FINDING_VERIFICATION — the
  Finding relationship always resolves through the authoritative
  rework-cycle / review row. A broken or missing parent fails closed
  (404) and an inaccessible parent Building is rejected (403); the
  non-Finding kinds keep `finding`/`rework`/`verification` `null`,
...
- `resourceType` / `operation`:
  ...
  - `EVIDENCE_SUBMISSION` — `SUBMIT` (resourceId = executionId; data: the
    ...
    `fileReference`/`mimeType`/`fileSize`/…; the exact permission is derived
    from `executionType` — MOB-C06 PART 01A: `FINDING_VERIFICATION` →
    `finding.review`, other kinds → `evidence.manage`),
...
- **Scope** — BE-02G preserved: review targets assert the accessible-Client
  set; a task-bound `FORM_INSTANCE` also derives Building from the exact
  generated task (MOB-C07 PART 05) and fails closed on a broken parent
  chain; FINDING asserts Building access (like the Web finding-verification
  endpoint). Returns 201 with the refreshed contract after a submit.
  `form_instance.execute` does not grant review.
