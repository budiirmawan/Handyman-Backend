...
/**
 * Security invariant for the safe public user representation (BE-01A/BE-01C):
 * the core public identity fields must be present and credential / password /
 * token / hash / secret fields must NEVER be exposed. We intentionally do NOT
 * require an exact complete key set so legitimate additive PUBLIC metadata
 * (e.g. the WhatsApp contact + consent fields added by CR-BE-NOTIFY-PROV-01
 * PART 06) is allowed to appear without weakening the leak protection.
 */
function assertSafePublicUser(user: Record<string, unknown>): void {
  for (const required of PUBLIC_USER_KEYS) {
    assert.ok(
      Object.keys(user).includes(required),
      `public user must include field ${required}`,
    );
  }

  const sensitive = Object.keys(user).filter((key) =>
    /password|passwd|credential|secret|token|hash|salt/i.test(key),
  );
  assert.deepEqual(
    sensitive,
    [],
    `public user must not expose sensitive fields, got: ${sensitive.join(', ')}`,
  );
}
...
describe('POST /api/v1/auth/login', () => {
  it('logs in an ACTIVE user with valid credentials', async (t) => {
    ...
    assertSafePublicUser(responseUser);
...
describe('GET /api/v1/auth/me', () => {
  it('returns safe identity for a valid session', async (t) => {
    ...
    assertSafePublicUser(response.body.data.user);
