...
describe('CR-BE-MOB-01 PART 06 — supported sync resource kinds', () => {
  ...
  it('enforces the same RBAC permission as the published REST route', () => {
    ...
    // MOB-C06 PART 01A — kinds whose exact permission is derived per
    // operation (documented via permissionByExecutionType) must be wired to
    // the shared derivation in the dispatcher, never to a client-supplied
    // permission value.
    const derived = syncOp['x-sync-supported-resource-types'].filter(
      (e: any) => typeof e.permissionByExecutionType === 'string',
    );
    assert.equal(
      derived.length,
      1,
      'only EVIDENCE_SUBMISSION documents a per-executionType permission',
    );
    assert.equal(derived[0].resourceType, 'EVIDENCE_SUBMISSION');
    assert.match(
      derived[0].permissionByExecutionType,
      /FINDING_VERIFICATION → finding\.review/,
      'the documented derivation must map FINDING_VERIFICATION to finding.review',
    );
    assert.match(
      service,
      /evidenceUploadPermissionFor\(operation\.data\?\.executionType\)/,
      'the dispatcher must derive the EVIDENCE_SUBMISSION permission from the operation executionType',
    );
    assert.match(
      readSource('src/modules/evidence/evidence.service.ts'),
      /FINDING_VERIFICATION: 'finding\.review'/,
      'the shared evidence permission map must require finding.review for FINDING_VERIFICATION',
    );
