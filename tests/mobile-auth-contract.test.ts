...
describe('CR-BE-MOB-CONTRACT-01 PART 01 — auth/session OpenAPI contract', () => {
  ...
  it('never advertises a refresh token, a refresh endpoint, or a sessionId', () => {
    // Only the three session routes exist in the runtime auth router; the
    // contract must not publish a token refresh/rotation path.
    const authPaths = Object.keys(spec.paths ?? {}).filter((p) =>
      p.startsWith('/auth/'),
    );
    assert.deepEqual(
      authPaths.sort(),
      ['/auth/audit-events', '/auth/login', '/auth/logout', '/auth/me', '/auth/me/buildings'],
      'no refresh/extra auth route may be documented',
    );
    assert.ok(
      !authPaths.some((p) => /refresh/i.test(p)),
      'no /auth/refresh route may be documented',
    );

    const loginResponse = spec.components.schemas.LoginResponse;
    for (const forbidden of ['refreshToken', 'accessToken', 'sessionId']) {
      assert.ok(
        !loginResponse.properties[forbidden],
        `LoginResponse must not expose ${forbidden}`,
      );
    }
    // The backend session credential stays named `sessionToken` (mobile maps
    // it to its own accessToken field in its adapter layer).
    assert.ok(loginResponse.properties.sessionToken, 'sessionToken is the contract name');
  });

  it('documents server-authoritative X-Request-ID (never a client echo)', () => {
    const header = spec.components.headers.RequestId;
    assert.ok(header, 'RequestId header component required');
    const text = JSON.stringify(header);
    assert.match(text, /server-generated/i, 'X-Request-ID is server-generated');
    assert.match(
      text,
      /never taken from a caller/i,
      'X-Request-ID must never be adopted from a caller-supplied header',
    );
  });

  it('documents the 403 codes the RBAC/scope runtime emits', () => {
    const forbidden = spec.components.responses.Forbidden;
    assert.ok(forbidden, 'Forbidden response required');
    const examples = forbidden.content['application/json'].examples;
    assert.deepEqual(
      Object.keys(examples).sort(),
      ['buildingAccessDenied', 'permissionDenied'],
      'Forbidden must show PERMISSION_DENIED and BUILDING_ACCESS_DENIED',
    );
    assert.equal(
      examples.permissionDenied.value.error.code,
      'PERMISSION_DENIED',
    );
    assert.equal(
      examples.buildingAccessDenied.value.error.code,
      'BUILDING_ACCESS_DENIED',
    );
    // The session endpoints (login/me/logout) never return 403 themselves;
    // 403 arises from RBAC/scope middleware on protected resources.
    for (const [path, method] of [
      ...
      assert.ok(
        !pathOperation(spec, path, method).responses['403'],
        `${method.toUpperCase()} ${path} must not document a 403 response`,
      );
    }
  });
});
