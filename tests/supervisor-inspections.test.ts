...
describe('BE-11G Supervisor Inspection Binding operations', () => {
  ...
  it('enforces RBAC and Building isolation', async (t) => {
    ...
    // DEF-J05-F-01 — the rejected request must leave zero authoritative rows.
    const leakedDailyInspections = await pool!.query<{ count: number }>(
      `SELECT COUNT(*)::int AS count FROM supervisor_inspections
       WHERE target_type = 'DAILY_CLEANING' AND target_id = $1`,
      [fixture.task.id],
    );
    assert.equal(
      leakedDailyInspections.rows[0].count,
      0,
      'no supervisor inspection persisted from rejected cross-building request',
    );
  });

  it('denies cross-building creation before any inspection or shared review persists', async (t) => {
    if (!requireDatabase(t)) return;

    const fixture = await createStructureFixture();

    // Actor holds supervisor_inspection.manage (passes route RBAC) but has no
    // assignment to the target Building. TOILET_INSPECTION exercises the
    // shared BE-07 review creation path (the worst-case mutation).
    const outsider = await createAdminUser();
    const denied = await api()
      .post('/api/v1/housekeeping/supervisor-inspections')
      .set(authHeaders(outsider.token))
      .send({
        targetType: 'TOILET_INSPECTION',
        targetId: fixture.toiletExecutionId,
        notes: 'unauthorized cross-building attempt',
      });

    assert.equal(denied.status, 403);
    assert.equal(denied.body.error.code, 'BUILDING_ACCESS_DENIED');

    // Zero supervisor inspection rows persisted for the target.
    const inspections = await pool!.query<{ count: number }>(
      `SELECT COUNT(*)::int AS count FROM supervisor_inspections
       WHERE target_type = 'TOILET_INSPECTION' AND target_id = $1`,
      [fixture.toiletExecutionId],
    );
    assert.equal(
      inspections.rows[0].count,
      0,
      'no supervisor inspection persisted from rejected request',
    );

    // Zero shared review rows persisted from the rejected request.
    const reviews = await pool!.query<{ count: number }>(
    ...
    assert.equal(
      ...
      0,
      'no shared review persisted from rejected request',
    );
  });
});
