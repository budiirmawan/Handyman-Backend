...
describe('patrol route RBAC and building isolation', () => {
  ...
  it('denies patrol-route-point PATCH across the building boundary and persists zero mutation (DEF-J08-F-01)', async (t) => {
    if (!requireDatabase(t)) return;
    ...
    const point = await api()
      .post(`/api/v1/security/patrol-routes/${route.body.data.id}/points`)
      .set(authHeaders())
      .send({ sequence: 1, areaId: area.id, notes: 'Original checkpoint' });
    assert.equal(point.status, 201);
    const pointId = point.body.data.id;

    // Outsider holds patrol_route.manage (passes route RBAC) but has no
    // assignment to the checkpoint's Building.
    const outsider = await createAdminUser();
    ...
    // The rejected request must leave the checkpoint field-equivalent.
    const persisted = await pool!.query<{
      ...
      `SELECT sequence, notes, status, area_id
       FROM patrol_route_points WHERE id = $1`,
      [pointId],
    );
    assert.equal(persisted.rows[0].sequence, 1, 'sequence unchanged');
    assert.equal(
      persisted.rows[0].notes,
      'Original checkpoint',
      'notes unchanged',
    );
    assert.equal(persisted.rows[0].status, 'ACTIVE', 'status unchanged');
    assert.equal(
      persisted.rows[0].area_id,
      area.id,
      'location references unchanged',
    );
  });
});
