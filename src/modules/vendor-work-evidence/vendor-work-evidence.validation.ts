...
import {
  ...
  type SubmitVendorWorkEvidenceInput,
  type VendorWorkEvidenceFilters,
  type VendorWorkEvidenceType,
...
export function parseSubmitVendorWorkEvidenceBody(
  body: unknown,
): Omit<
  SubmitVendorWorkEvidenceInput,
  'vendorWorkId' | 'clientId' | 'submittedByUserId'
> {
  if (!isRecord(body)) {
    throw AppError.validation('Request validation failed.', [
...
/**
 * Parses list filters (`GET /vendor-work-evidence?...`). At least one of
 * `vendorWorkId`, `vendorId`, or `buildingId` must be supplied.
 */
export function parseVendorWorkEvidenceFilters(
  query: Record<string, unknown>,
): VendorWorkEvidenceFilters {
  const details: ValidationDetail[] = [];

  const vendorWorkId = readOptionalUuid(
    query.vendorWorkId,
    'vendorWorkId',
    details,
  );
...
): VendorWorkEvidenceType | undefined {
  if (!isVendorWorkEvidenceType(value)) {
    details.push({
      field: 'evidenceType',
      message: `Evidence type must be one of: ${VENDOR_WORK_EVIDENCE_TYPES.join(', ')}.`,
    });
    return undefined;
  }
  return value;
