...
export async function loadReviewTarget(
  ...
  const result = await getPool().query<{
    ...
    targetType === 'FORM_INSTANCE'
      ? `SELECT id, client_id, status, form_template_version_id, generated_task_id
           FROM form_instances WHERE id = $1`
      : `SELECT id, client_id, status FROM checklist_executions WHERE id = $1`,
    [targetId],
  );
  ...
  if (targetType === 'FORM_INSTANCE') {
    const buildingId = await resolveBoundFormInstanceBuilding({
