...
import { isVendorWorkStatus } from '../vendor-work';
import { vendorServiceRegisterRepository } from './vendor-service-register.repository';
import type {
...
/**
 * CR-BE-REPORT-READ-01 PART 01 — Vendor Service Register service.
 *
 ...
 * is sufficient.
 *
 * SCOPE — mirrors the BE-23H read-model behavior exactly: an explicit
 * `buildingId` is existence-checked and access-asserted; an omitted one
 * rolls the register up across exactly the Buildings the caller can reach,
 * so cross-Building and cross-Client leakage is impossible; an empty
 ...
 * status/decision values (validated by the authorities' own validators,
 * never reinterpreted) and the BE-23H UTC half-open date-window
 * convention applied to the assignment instant.
 */
...
export function parseVendorServiceRegisterQuery(
  ...
  const vendorWorkStatus = readOptionalEnum(
    query.vendorWorkStatus,
    'vendorWorkStatus',
    isVendorWorkStatus,
    'vendorWorkStatus must be a valid vendor-work status.',
    details,
  );
  ...
  return {
    ...
    ...(vendorWorkStatus === undefined ? {} : { vendorWorkStatus }),
...
/**
 * Converts the register window into a half-open [start, end) range. A
 * date-only `dateTo` is inclusive of that whole UTC day. Mirrors
 * `vendorTenantKpiRange`.
 */
export function vendorServiceRegisterRange(filters: VendorServiceRegisterFilters): {
  start: Date | null;
  end: Date | null;
} {
...
async function resolveScope(
  ...
  const range = vendorServiceRegisterRange(filters);
...
export async function getVendorServiceRegister(
  filters: VendorServiceRegisterFilters,
  userId: string,
): Promise<PublicVendorServiceRegister> {
  ...
  const rows =
    await vendorServiceRegisterRepository.getVendorServiceRegisterRows(
      buildingIds,
      filters,
      start,
      end,
...
export const vendorServiceRegisterService = {
