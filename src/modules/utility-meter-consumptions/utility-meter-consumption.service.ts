...
} from '../utility-aggregations/utility-aggregation.types';
import {
...
/**
 * Bounded actor-scoped Building × Period read over persisted consumption
 * facts. The repository performs the only grouped SQL query; this service
 * resolves access and validates the frozen interval/type vocabularies.
 */
export async function readUtilityConsumptionTrend(
  request: UtilityConsumptionTrendReadRequest,
  actorUserId: string,
): Promise<PublicUtilityConsumptionTrendBucket[]> {
  const details: Array<{ field: string; message: string }> = [];
  const requestedBuildingIds = Array.isArray(request.buildingIds)
    ? [
        ...new Set(
          request.buildingIds.map((id) =>
  ...
  if (
    ...
    requestedBuildingIds.some((id) => !isValidUuid(id))
  ) {
    details.push({
      field: 'buildingIds',
  ...
  if (typeof actorUserId !== 'string' || actorUserId.trim() === '') {
    details.push({ field: 'actorUserId', message: 'actorUserId is required.' });
  }
  if (!(request.periodStart instanceof Date) || Number.isNaN(request.periodStart.getTime())) {
    details.push({ field: 'periodStart', message: 'periodStart must be a valid date.' });
  }
  if (!(request.periodEnd instanceof Date) || Number.isNaN(request.periodEnd.getTime())) {
    details.push({ field: 'periodEnd', message: 'periodEnd must be a valid date.' });
  }
  if (
    request.interval === undefined ||
    !isUtilityAggregationInterval(request.interval)
  ) {
    details.push({
      field: 'interval',
      message: `interval must be one of: ${UTILITY_AGGREGATION_INTERVALS.join(', ')}.`,
    });
  }

  let utilityTypes: UtilityType[] | undefined;
  if (request.utilityTypes !== undefined) {
    if (!Array.isArray(request.utilityTypes)) {
      details.push({
        field: 'utilityTypes',
        message: `utilityTypes must contain only: ${UTILITY_TYPES.join(', ')}.`,
      });
    } else if (request.utilityTypes.some((value) => !isUtilityType(value))) {
      details.push({
        field: 'utilityTypes',
        message: `utilityTypes must contain only: ${UTILITY_TYPES.join(', ')}.`,
      });
    } else {
      utilityTypes = [...new Set(request.utilityTypes)];
    }
  }
  ...
  if (request.periodEnd < request.periodStart) {
    throw AppError.validation('Request validation failed.', [
      { field: 'periodEnd', message: 'periodEnd must be on or after periodStart.' },
    ]);
  }

  const accessibleBuildingIds =
    await contextAccessService.getAccessibleBuildingIds(actorUserId);
  const requestedSet = new Set(requestedBuildingIds);
  const effectiveBuildingIds = accessibleBuildingIds.filter((id) =>
    requestedSet.has(id.toLowerCase()),
  );
  if (effectiveBuildingIds.length === 0) return [];

  const rows = await utilityMeterConsumptionRepository.readConsumptionTrend(
