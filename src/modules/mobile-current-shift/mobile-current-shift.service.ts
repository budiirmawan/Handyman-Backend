...
type CurrentShiftRow = {
  ...
  end_time: string;
  shift_status: string;
  /** MOB-C03 PART 03B — nullable Security Post summary fields */
  security_post_id: string | null;
  security_post_code: string | null;
  security_post_name: string | null;
};
...
function toMobileShift(
  ...
  return {
    ...
    effectiveUntil: row.effective_until
      ...
      : null,
    securityPost:
      row.security_post_id && row.security_post_code && row.security_post_name
        ? {
            id: row.security_post_id,
            code: row.security_post_code,
            name: row.security_post_name,
          }
        : null,
  };
}
