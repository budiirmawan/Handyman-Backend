...
import { assertPermitWorkActionAllowed, resolvePermitWorkAuthority, type PermitWorkAuthorityResolver } from './permit-work-lifecycle.authority';
import {
...
export async function resolvePermitWorkStartReadiness(permitId:string,actorUserId:string,resolveAuthority:PermitWorkAuthorityResolver=resolvePermitWorkAuthority):Promise<PermitWorkStartReadiness>{
 const authority=await resolveAuthority(permitId,actorUserId);
 if(!authority.readiness){
  const status=await getPermitWorkStatus(permitId,actorUserId,resolveAuthority);
...
export async function startPermitWork(permitId:string,input:PermitWorkNotesInput,actorUserId:string,resolveAuthority:PermitWorkAuthorityResolver=resolvePermitWorkAuthority):Promise<PublicPermitWorkLifecycle>{
 ...
 await assertPermitWorkActionAllowed(permitId,actorUserId,'START',resolveAuthority);
...
export async function closePermitWork(permitId:string,input:PermitWorkNotesInput,actorUserId:string,resolveAuthority:PermitWorkAuthorityResolver=resolvePermitWorkAuthority):Promise<PublicPermitWorkLifecycle>{
 const existing=await permitWorkLifecycleRepository.findByPermitId(permitId);
 ...
 await assertPermitWorkActionAllowed(permitId,actorUserId,'CLOSE',resolveAuthority);
...
export async function getPermitWorkStatus(permitId:string,actorUserId:string,resolveAuthority:PermitWorkAuthorityResolver=resolvePermitWorkAuthority):Promise<PermitWorkStatusView>{
 ...
 const record=await permitWorkLifecycleRepository.findByPermitId(permit.id),authority=await resolveAuthority(permit.id,actorUserId),status=permit.status==='CANCELLED'?'CANCELLED':record?.status??'READY';
