...
import {
  ...
  REPORTING_EXPORT_DATASET_METADATA,
...
function selectTable(
  ...
  const defaultTableKey =
    snapshot.metadata?.csvDefaultTableKey ??
    (snapshot.metadata
      ? REPORTING_EXPORT_DATASET_METADATA[snapshot.metadata.dataset]?.csvDefaultTableKey
      : undefined);
  if (defaultTableKey !== undefined) {
    const defaultTable = snapshot.tables.find(
      (candidate) => candidate?.key === defaultTableKey,
    );
    if (!defaultTable) {
      throw new CsvRendererError(
        'TABLE_NOT_FOUND',
        `No governed CSV table exists for tableKey ${JSON.stringify(defaultTableKey)}.`,
      );
    }
    return defaultTable;
  }
