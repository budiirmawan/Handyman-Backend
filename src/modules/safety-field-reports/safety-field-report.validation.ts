...
import {
  SAFETY_REPORT_TYPES,
  isSafetyReportType,
  type CreateSafetyFieldReportInput,
...
export function parseSafetyFieldReportIncidentIdParam(raw: string): string {
...
export function parseCreateSafetyFieldReportBody(
  body: unknown,
  ...
  if (controlled) {
    fail([
      {
        field: controlled,
        message: 'This field is derived or not accepted by this endpoint.',
      },
    ]);
  }

  const details: ValidationDetail[] = [];
  const reportType = readReportType(body.reportType, details);
...
function readReportType(
  ...
  const normalized = typeof value === 'string'
    ...
    : value;
  if (!isSafetyReportType(normalized)) {
    details.push({
      field: 'reportType',
      message: `reportType must be one of: ${SAFETY_REPORT_TYPES.join(', ')}.`,
    });
    return undefined;
  }
  return normalized;
