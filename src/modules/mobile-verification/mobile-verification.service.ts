...
async function loadReviewTargetResource(
  ...
  targetId: string,
  userId: string,
): Promise<{ status: string; client_id: string; reference: MobileVerificationResourceReference }> {
  ...
  }>(`SELECT id, client_id, status, started_at, completed_at FROM ${table} WHERE id = $1`, [
    targetId,
  ]);
  ...
  return { status: row.status, client_id: row.client_id, reference };
}
...
function buildReviewTargetContract(
  targetType: 'CHECKLIST_EXECUTION' | 'FORM_INSTANCE',
  targetId: string,
  resource: { status: string; client_id: string; reference: MobileVerificationResourceReference },
  reviews: PublicReview[],
