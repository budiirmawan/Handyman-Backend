...
const REQUIRED_PERMISSION: Record<MobileSyncResourceType, string> = {
  ...
  // MOB-C06 PART 01A — the DEFAULT permission for EVIDENCE_SUBMISSION. The
  // exact permission is derived per operation from its executionType (see
  // executeOperation): FINDING_VERIFICATION → finding.review; FINDING /
  // FINDING_REWORK / existing kinds → evidence.manage.
...
async function executeOperation(
  ...
  try {
    // MOB-C06 PART 01A — EVIDENCE_SUBMISSION derives the EXACT permission
    // from the operation's own executionType (never from a client-supplied
    // permission value): FINDING_VERIFICATION → finding.review (generic
    // POST /evidence + dedicated-route + mobile-upload parity); every other
    // known kind → evidence.manage. An unknown/missing executionType keeps
    // the evidence.manage default and then fails closed in the shared
    // service validation (per-item FAILED, no mutation).
    const required =
      operation.resourceType === 'EVIDENCE_SUBMISSION'
        ? evidenceUploadPermissionFor(operation.data?.executionType) ??
          REQUIRED_PERMISSION.EVIDENCE_SUBMISSION
        : REQUIRED_PERMISSION[operation.resourceType];
