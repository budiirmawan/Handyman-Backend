...
/**
 * Reads persisted metric values for an already-authorized Building set.
 *
 * The period window uses the domain's existing containment semantics: both
 * persisted endpoints must lie within the requested bounds. The query is one
 * set-based statement and never aggregates or recalculates values.
 */
async function listBounded(
  ...
  if (accessibleBuildingIds.length === 0) return [];
  if (metricDefinitionIds !== undefined && metricDefinitionIds.length === 0) return [];

  const conditions = [
    'building_id = ANY($1::uuid[])',
    'period_start >= $2',
    'period_end <= $3',
  ];
  const values: unknown[] = [accessibleBuildingIds, periodStart, periodEnd];

  if (metricDefinitionIds !== undefined) {
    conditions.push('metric_definition_id = ANY($4::uuid[])');
    values.push(metricDefinitionIds);
  }

  const result = await executor.query<Row>(
    `SELECT ${SELECT} FROM esg_metric_values
