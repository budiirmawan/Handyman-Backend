...
import { parseEsgWasteRecordFilters } from './esg-waste-record.validation';
import type {
...
/**
 * Bounded source read for actor-scoped ESG waste register consumers.
 *
 * The existing endpoint read is intentionally left unchanged. This seam owns
 * the requested Building-set intersection and reuses the existing repository
 * set query plus its inclusive DATE period filters; it performs no per-Building
 * fanout and accepts no caller Client scope.
 */
export async function readEsgWasteRecords(
  request: EsgWasteRecordReadRequest,
  actorUserId: string,
): Promise<PublicEsgWasteRecord[]> {
  ...
  if (typeof request.periodStart !== 'string' || request.periodStart.trim() === '') {
    details.push({ field: 'periodStart', message: 'periodStart is required.' });
  }
  if (typeof request.periodEnd !== 'string' || request.periodEnd.trim() === '') {
    details.push({ field: 'periodEnd', message: 'periodEnd is required.' });
  }
  if (details.length > 0) {
    throw AppError.validation('Request validation failed.', details);
  }

  const bounded = parseEsgWasteRecordFilters({
    dateFrom: request.periodStart,
    dateTo: request.periodEnd,
  });
  if (
    bounded.dateFrom === undefined ||
    bounded.dateTo === undefined ||
    bounded.dateFrom > bounded.dateTo
  ) {
  ...
  const accessibleBuildingIds = await contextAccessService.getAccessibleBuildingIds(
    actorUserId,
  );
  ...
  const records = await esgWasteRecordRepository.listScoped(
    undefined,
    effectiveBuildingIds,
    { dateFrom: bounded.dateFrom, dateTo: bounded.dateTo },
  );
